...
static void levelMotorGet(struct bt_mesh_lvl_srv *srv, struct bt_mesh_msg_ctx *ctx, struct bt_mesh_lvl_status *rsp)
{
	struct btMeshlevelMotor *levelMotor = CONTAINER_OF(srv, struct btMeshlevelMotor, srvLvl);
	int Motorid = levelMotor - &levelMotors[0];
	LOG_INF("Received [level][GET] op[0x%06x] LevelMotor[%d] from addr:%d revc_addr:%d rssi:%d tid:%d", srv->model->op->opcode, Motorid, ctx->addr, ctx->recv_dst, ctx->recv_rssi, srv->tid.tid);
	...
	rsp->remaining_time = k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&levelMotor->levelMotorDelayWork)) + levelMotor->remainingTime;

	// Set the current and target levels for the response
	rsp->current = levelMotor->currentLvl;
	rsp->target = levelMotor->targetLvl;
}
static void levelMotorSet(struct bt_mesh_lvl_srv *srv, struct bt_mesh_msg_ctx *ctx, const struct bt_mesh_lvl_set *set, struct bt_mesh_lvl_status *rsp)
{
	struct btMeshlevelMotor *levelMotor = CONTAINER_OF(srv, struct btMeshlevelMotor, srvLvl);
	int Motorid = levelMotor - &levelMotors[0];
	LOG_INF("Received [level][SET] levelMotors[%d] from addr:%d to:%d rssi:%d tid:%d", Motorid, ctx->addr, ctx->recv_dst, ctx->recv_rssi, srv->tid.tid);
...
static void levelMotorDelta(struct bt_mesh_lvl_srv *srv, struct bt_mesh_msg_ctx *ctx, const struct bt_mesh_lvl_delta_set *delta_set, struct bt_mesh_lvl_status *rsp)
{
	LOG_DBG("level delta");
}
static void levelMotorMove(struct bt_mesh_lvl_srv *srv, struct bt_mesh_msg_ctx *ctx, const struct bt_mesh_lvl_move_set *move_set, struct bt_mesh_lvl_status *rsp)
{
	LOG_DBG("level move");
}

const struct bt_mesh_lvl_srv_handlers levelMotorHandlers = {
	.set = levelMotorSet,
	.get = levelMotorGet,
...
void levelMotorWork(struct k_work *work)
{
	...
	struct btMeshlevelMotor *levelMotor = CONTAINER_OF(work, struct btMeshlevelMotor, levelMotorDelayWork);

	if (levelMotor->remainingTime)
	{
		k_work_reschedule(&levelMotor->levelMotorDelayWork, K_MSEC(levelMotor->remainingTime));
	...
	else
	{
		...
		LOG_INF("Send [level][STATUS] op[0x%06x] from addr:0x%04x.to 0x%04x.   tid:-- ", BT_MESH_LVL_OP_STATUS, bt_mesh_model_elem(levelMotor->srvLvl.model)->addr, levelMotor->srvLvl.pub.addr);
