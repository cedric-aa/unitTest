			...
			struct net_buf_simple *buf)
{
	...
	LOG_DBG("Result timerIsActive: [%s] Time remaining: [%d]", timerIsActive ? "true" : "false",
		timeRemaining);
...
static int activationSetPwd(struct btMeshActivation *activation, bool timerIsActive, uint8_t pwd)
{
