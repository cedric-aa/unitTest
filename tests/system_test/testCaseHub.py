...
def packWithAddress(address: int, msgData: bytes, sequenceNumber: bytes, uart_ack_byte: bytes) -> bytes:
    payload = uart_ack_byte + \
        address.to_bytes(2, 'big') + msgData + sequenceNumber
...
def send_command1(command):
    ...
    data = b''.join([byte.to_bytes(1, 'big')
                    for byte in command.get("data", [])])
    ...
    hex_message = ' '.join(
        ['0x' + byte.to_bytes(1, 'big').hex() for byte in message])
...
def handle_unit_control(message_id, data_bytes, sequence_number):
    if message_id == 0x01:
        ...
        print("Status Code Unit Control: Error Code = ", error_code,
              "Sequence Number =", int.from_bytes(sequence_number))
...
def handle_motor_control(message_id, data_bytes, sequence_number):
    if message_id == 0x01:
        ...
        error_code = data_bytes[0]
        print("Status Code Motor: Error Code = ", error_code,
              "Sequence Number =", int.from_bytes(sequence_number))
...
def handle_activation_control(message_id, data_bytes, sequence_number):
    if message_id == 0x03:
        ...
        error_code = data_bytes[0]
        print("Status Code Activation: Error Code = ", error_code,
              "Sequence Number =", int.from_bytes(sequence_number))
...
def process_response(response):
   # print("Received response:", response)
    ...
    # Extract message type as an integer
    message_type = int.from_bytes(response[5:6], 'big')
    # Extract message ID as an integer
    message_id = int.from_bytes(response[6:7], 'big')
    ...
    response_dict = {"message_type": message_type, "message_id": message_id,
                     "data": data_list, "sequence_number": int.from_bytes(sequence_number)}
...
def run_test_case(test_case, sequence_number):
    ...
    if response is None:
        print("\033[31mTest failed: No response received within the timeout message SequenceNumber:",
              sequence_number, " \033[0m")
        return
    if (response["message_type"] == test_case["expected_response"]["message_type"] and
        ...
        print("\033[31mTest failed: Expected:",
              test_case["expected_response"], "Received:", response, "\033[0m")
